...
use sage_core::database::{Theoretical, PeptideIx, Parameters, EnzymeBuilder, Builder, IndexedDatabase};
...
impl PyIndexedDatabase {
    ...
    pub fn new(
        ...
        Ok(PyIndexedDatabase { inner: IndexedDatabase {
            ...
            min_value,
            potential_mods: potential_mods.into_iter().map(|(k, v)| (k.inner, v)).collect(),
            bucket_size,
            generate_decoys,
            decoy_tag,
        } })
    ...
    pub fn from_parameters(parameters: PyParameters, fasta: PyFasta) -> PyResult<Self> {
        Ok(PyIndexedDatabase { inner: parameters.inner.build(fasta.inner) })
    }

    pub fn query(&self, precursor_mass: f32, precursor_tolerance: PyTolerance, fragment_tolerance: PyTolerance) -> PyResult<PyIndexedQuery> {
    ...
    pub fn peptides(&self) -> Vec<PyPeptide> {
        self.inner.peptides.iter().map(|p| PyPeptide { inner: p.clone() }).collect()
    }

    pub fn peptides_as_string(&self, _py: Python) -> Vec<String> {
        let peptides = self.inner.peptides.iter().map(|p| String::from_utf8(p.sequence.clone().to_vec())).collect::<Result<Vec<String>, _>>().unwrap();
        peptides
    }

    pub fn mono_masses(&self, py: Python) -> Py<PyArray1<f32>> {
        let masses = self.inner.peptides.iter().map(|p| p.monoisotopic).collect::<Vec<f32>>();
    ...
    pub fn fragments(&self) -> Vec<PyTheoretical> {
        self.inner.fragments.iter().map(|f| PyTheoretical { inner: f.clone() }).collect()
    }
    ...
    pub fn ion_kinds(&self) -> Vec<PyKind> {
        self.inner.ion_kinds.iter().map(|k| PyKind { inner: k.clone() }).collect()
    }
    ...
    pub fn potential_mods(&self) -> Vec<(PyModificationSpecificity, f32)> {
        self.inner.potential_mods.iter().map(|(k, v)| (PyModificationSpecificity { inner: k.clone() }, *v)).collect()
    }
...
impl PyEnzymeBuilder {
    ...
    pub fn new(
        ...
        Ok(PyEnzymeBuilder { inner: EnzymeBuilder {
            missed_cleavages,
            min_len,
            max_len,
            cleave_at,
    ...
    pub fn from_default_trypsin() -> PyResult<Self> {
        Ok(PyEnzymeBuilder { inner: EnzymeBuilder::default() })
    }

    pub fn get_enzyme_parameters(&self) -> PyResult<PyEnzymeParameters> {
        Ok(PyEnzymeParameters { inner: self.clone().inner.into() })
    }
...
impl PyPeptideIx {
    ...
    pub fn new(idx: u32) -> PyResult<Self> {
        Ok(PyPeptideIx { inner: PeptideIx(idx) })
    }
...
impl PyTheoretical {
    ...
    pub fn new(idx: u32, fragment_mz: f32) -> PyResult<Self> {
        Ok(PyTheoretical { inner: Theoretical {peptide_index: PeptideIx(idx), fragment_mz } })
    }

    #[getter]
    pub fn idx(&self) -> PyPeptideIx {
        PyPeptideIx { inner: self.inner.peptide_index }
    }
...
impl PyParameters {
    ...
    pub fn new(
        ...
        Ok(PyParameters { inner: Parameters {
            bucket_size,
            ...
            peptide_min_mass,
            peptide_max_mass,
            ion_kinds: ion_kinds.map(|k| k.iter().map(|k| k.inner.clone()).collect()).unwrap_or_else(|| vec![Kind::B, Kind::Y]),
            min_ion_index,
            static_mods: static_mods.extract::<HashMap<PyModificationSpecificity, f32>>().unwrap().iter().map(|(k, v)| (k.inner.clone(), *v)).collect(),
            variable_mods: variable_mods.extract::<HashMap<PyModificationSpecificity, Vec<f32>>>().unwrap().iter().map(|(k, v)| (k.inner.clone(), v.clone())).collect(),
            max_variable_mods,
            decoy_tag,
            generate_decoys,
            fasta,
    ...
    pub fn from_default() -> PyResult<Self> {
        Ok(PyParameters { inner: Builder::default().make_parameters() })
    }

    pub fn digest(&self) -> PyResult<Vec<PyPeptide>> {
        let fasta = Fasta::parse(self.inner.fasta.clone(), self.inner.decoy_tag.clone(), self.inner.generate_decoys);
    ...
    pub fn build_indexed_database(&self) -> PyResult<PyIndexedDatabase> {
        Ok(PyIndexedDatabase { inner: self.inner.clone().build(Fasta::parse(self.inner.fasta.clone(), self.inner.decoy_tag.clone(), self.inner.generate_decoys))})
    }
    ...
    pub fn enzyme_builder(&self) -> PyEnzymeBuilder {
        PyEnzymeBuilder { inner: self.inner.enzyme.clone() }
    }
    ...
    pub fn static_mods(&self) -> HashMap<PyModificationSpecificity, f32> {
        self.inner.static_mods.iter().map(|(k, v)| (PyModificationSpecificity { inner: k.clone() }, *v)).collect::<HashMap<PyModificationSpecificity, f32>>()
    }

    #[getter]
    pub fn variable_mods(&self) -> HashMap<PyModificationSpecificity, Vec<f32>> {
        self.inner.variable_mods.iter().map(|(k, v)| (PyModificationSpecificity { inner: k.clone() }, v.clone())).collect::<HashMap<PyModificationSpecificity, Vec<f32>>>()
    }
