...
use pyo3::prelude::*;
use sage_core::modification::{ ModificationSpecificity, InvalidModification, validate_mods };
use std::str::FromStr;
...
impl PyModificationSpecificity {
    ...
    pub fn new(s: &str) -> PyResult<Self> {
        match ModificationSpecificity::from_str(s) {
            ...
            Err(InvalidModification::Empty) => Err(PyValueError::new_err("Empty modification string")),
            Err(InvalidModification::InvalidResidue(c)) => Err(PyValueError::new_err(format!("Invalid modification string: unrecognized residue ({})", c))),
            Err(InvalidModification::TooLong(s)) => Err(PyValueError::new_err(format!("Invalid modification string: {} is too long", s))),
        }
    }
...
pub fn py_validate_mods(input: Option<&PyDict>) -> HashMap<PyModificationSpecificity, f32> {
    ...
    let py_validated_mods = output.iter().map(|(k, v)| (PyModificationSpecificity {inner: k.clone()}, *v)).collect::<HashMap<PyModificationSpecificity, f32>>();

    py_validated_mods
}

#[pyfunction]
pub fn py_validate_var_mods(input: Option<&PyDict>) -> HashMap<PyModificationSpecificity, Vec<f32>> {
    // unwrap the input
